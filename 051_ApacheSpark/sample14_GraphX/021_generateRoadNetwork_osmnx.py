# =============================================================
#  実在の道路ネットワークを osmnx で取得してグラフ化する
#
#  滋賀県全土の幹線道路（高速・国道・主要地方道クラス）を対象に、
#  取得〜グラフ化を osmnx にまかせる。
#  osmnx は交差点抽出・通過点の簡約・距離(length)付与まで自動でやってくれるので、
#  Overpass の生データを自前で解析する必要がない。
#
#  出力:
#    02X_input/nodes.csv : id, lat, lon
#    02X_input/edges.csv : src, dst, length_m, name, highway
# =============================================================
import os
import osmnx as ox
import pandas as pd
...
# 幹線道路クラス。"motorway|trunk|primary|secondary" は _link も含む。
