# =============================================================
#  osmnx で取った道路ネットワークを地図状に可視化する（Pattern 3 / 2番系）
#
...
#  ※ 先に 22 を spark-submit で実行して vertex_metrics.csv を作っておくこと。
# =============================================================
import os
import sys
import math
...
def main():
    if not os.path.exists(METRICS_CSV):
        sys.exit(
            "分析結果 {0} が見つかりません。\n"
            "先に  spark-submit --packages graphframes:... 22_road_analysis.py  を実行してください。".format(METRICS_CSV)
        )
    ...
    fig.suptitle("Real road network (Shiga Prefecture, via osmnx) — before / after analysis",
